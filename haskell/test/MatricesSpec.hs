...
matricesBasics =
  describe "Basics" $ do
    ...
    describe "Constructing and inspecting a 4x4 matrix" $ do
      let m = makeVMatrix [[1, 2, 3, 4], [5.5, 6.5, 7.5, 8.5],
                           [9, 10, 11, 12], [13.5, 14.5, 15.5, 16.5]]
      it "inspect [0,0] = 1" $ do
        SUT.getAtV m (RowIndex 0) (ColumnIndex 0) `shouldBe` 1

      it "inspect [0,3] = 4" $ do
        SUT.getAtV m (RowIndex 0) (ColumnIndex 3) `shouldBe` 4

      it "inspect [1,0] = 5.5" $ do
        SUT.getAtV m (RowIndex 1) (ColumnIndex 0) `shouldBe` 5.5

      it "inspect [1,2] = 7.5" $ do
        SUT.getAtV m (RowIndex 1) (ColumnIndex 2) `shouldBe` 7.5

      it "inspect [2,2] = 11" $ do
        SUT.getAtV m (RowIndex 2) (ColumnIndex 2) `shouldBe` 11

      it "inspect [3,0] = 13.5" $ do
        SUT.getAtV m (RowIndex 3) (ColumnIndex 0) `shouldBe` 13.5

      it "inspect [3,2] = 15.5" $ do
        SUT.getAtV m (RowIndex 3) (ColumnIndex 2) `shouldBe` 15.5
    ...
    describe "Constructing and inspecting a 2x2 matrix" $ do
      let m = makeVMatrix [[- 3, 5], [1, -2]]
      it "inspect [0,0] = -3" $ do
        SUT.getAtV m (RowIndex 0) (ColumnIndex 0) `shouldBe` (-3)

      it "inspect [0,1] = 5" $ do
        SUT.getAtV m (RowIndex 0) (ColumnIndex 1) `shouldBe` 5

      it "inspect [1,0] = 1" $ do
        SUT.getAtV m (RowIndex 1) (ColumnIndex 0) `shouldBe` 1

      it "inspect [1,1] = -2" $ do
        SUT.getAtV m (RowIndex 1) (ColumnIndex 1) `shouldBe` (-2)
    ...
    describe "Constructing and inspecting a 3x3 matrix" $ do
      let m = makeVMatrix [[- 3, 5, 0], [1, - 2, - 7], [0, 1, 1]]
      it "inspect [0,0] = -3" $ do
        SUT.getAtV m (RowIndex 0) (ColumnIndex 0) `shouldBe` (-3)
      ...
      it "inspect [2,2] = 1" $ do
        SUT.getAtV m (RowIndex 2) (ColumnIndex 2) `shouldBe` 1
    ...
    describe "Matrix equality with different matrices" $ do
      let a = makeVMatrix [[1, 2, 3, 4], [5, 6, 7, 8],
                           [9, 8, 7, 6], [5, 4, 3, 2]]
          b = makeVMatrix [[2, 3, 4, 5], [6, 7, 8, 9],
                           [8, 7, 6, 5], [4, 3, 2, 1]]
      it "A != B" $ do
        a /= b

    describe "Matrix equality uses epsilon comparison" $ do
      let a = makeVMatrix [[1.000001, 2, 3, 4], [5, 6, 7, 8],
                           [9, 8, 7, 6], [5, 4, 3, 2]]
          b = makeVMatrix [[1, 2, 3, 4], [5, 6, 7, 8],
                           [9, 8, 7, 6], [5, 4, 3, 2]]
      it "A == B" $ do
...
matricesArithmetic =
  describe "Matrix multiplication" $ do
    ...
    describe "Multiplying a matrix by a tuple" $ do
      let a = makeVMatrix [[1, 2, 3, 4], [2, 4, 4, 2],
                           [8, 6, 4, 1], [0, 0, 0, 1]]
          b = Tuple 1 2 3 1
          c = SUT.mulTV a b
      it "A * b = C" $ do
    ...
    describe "Multiplying a matrix by the identity matrix" $ do
      let a = makeVMatrix [[0, 1, 2, 4] , [1, 2, 4, 8],
                            [2, 4, 8, 16], [4, 8, 16, 32]]
          b = SUT.mulV a SUT.identityV
      it "A * Identity = A" $ do
        a `shouldBe` b
...
matrixFunctions =
  describe "Matrix Functions" $ do
    ...
    describe "Transpose a matrix" $ do
      let a = makeVMatrix [[0, 9, 3, 0], [9, 8, 0, 8],
                           [1, 8, 5, 3], [0, 0, 5, 8]]
          t = SUT.transposeV a
          b = makeVMatrix [[0, 9, 1, 0], [9, 8, 8, 0],
                           [3, 0, 5, 5], [0, 8, 3, 8]]
      it "Transpose A" $ do
        t `shouldBe` b
    ...
    describe "Transposing the identity matrix" $ do
      let t = SUT.transposeV SUT.identityV
      it "Transpose Identity is Identity" $ do
        t `shouldBe` SUT.identityV
    ...
    describe "Calculating the determinant of a 2x2 matrix" $ do
      let a = makeVMatrix [[1, 5], [- 3, 2]]
          d = SUT.determinantV a
    ...
    describe "A submatrix of a 3x3 matrix is a 2x2 matrix" $ do
      let a = makeVMatrix [[1, 5, 0], [- 3, 2, 7], [0, 6, - 3]]
          s = SUT.submatrixV a (RowIndex 0) (ColumnIndex 2)
          b = makeVMatrix [[- 3, 2], [0, 6]]
      it "is a 2x2" $ do
        s `shouldBe` b
    ...
    describe "A submatrix of a 4x4 matrix is a 3x3 matrix" $ do
      let a = makeVMatrix [[- 6, 1, 1, 6], [- 8, 5, 8, 6],
                           [- 1, 0, 8, 2], [- 7, 1, -1, 1]]
          s = SUT.submatrixV a (RowIndex 2) (ColumnIndex 1)
          b = makeVMatrix [[- 6, 1, 6], [- 8, 8, 6], [- 7, - 1, 1]]
      it "is a 3x3" $ do
        s `shouldBe` b
    ...
    describe "Calculating a minor of a 3x3 matrix" $ do
      let a = makeVMatrix [[3, 5, 0], [2, - 1, - 7], [6, - 1, 5]]
          b = SUT.submatrixV a (RowIndex 1) (ColumnIndex 0)
          d = SUT.determinantV b
          m = SUT.minorV a (RowIndex 1) (ColumnIndex 0)
    ...
    describe "Calculating a cofactor of a 3x3 matrix" $ do
      let a = makeVMatrix [[3, 5, 0], [2, - 1, - 7], [6, - 1, 5]]
          m1 = SUT.minorV a (RowIndex 0) (ColumnIndex 0)
          c1 = SUT.cofactorV a (RowIndex 0) (ColumnIndex 0)
          m2 = SUT.minorV a (RowIndex 1) (ColumnIndex 0)
          c2 = SUT.cofactorV a (RowIndex 1) (ColumnIndex 0)
    ...
    describe "Calculating the determinant of a 3x3 matrix" $ do
      let a  = makeVMatrix [[1, 2, 6], [- 5, 8, - 4], [2, 6, 4]]
          c1 = SUT.cofactorV a (RowIndex 0) (ColumnIndex 0)
          c2 = SUT.cofactorV a (RowIndex 0) (ColumnIndex 1)
          c3 = SUT.cofactorV a (RowIndex 0) (ColumnIndex 2)
          d  = SUT.determinantV a
    ...
    describe "Calculating the determinant of a 4x4 matrix" $ do
      let a  = makeVMatrix [[- 2, - 8, 3, 5], [- 3, 1, 7, 3],
                            [1, 2, - 9, 6], [- 6, 7, 7, -9]]
          c1 = SUT.cofactorV a (RowIndex 0) (ColumnIndex 0)
          c2 = SUT.cofactorV a (RowIndex 0) (ColumnIndex 1)
          c3 = SUT.cofactorV a (RowIndex 0) (ColumnIndex 2)
          c4 = SUT.cofactorV a (RowIndex 0) (ColumnIndex 3)
          d  = SUT.determinantV a
    ...
    describe "Testing an invertible matrix for invertibility" $ do
      let a = makeVMatrix [[6, 4, 4, 4], [5, 5, 7, 6],
                           [4, - 9, 3, - 7], [9, 1, 7, - 6]]
          d = SUT.determinantV a
      it "the determinant of A is -2120" $ do
        d `shouldBe` - 2120
      it "and A is invertible" $ do
        a `shouldSatisfy` SUT.invertibleV
    ...
    describe "Testing a noninvertible matrix for invertibility" $ do
      let a = makeVMatrix [[- 4, 2, - 2, - 3], [9, 6, 2, 6],
                            [0, - 5, 1, - 5], [0, 0, 0, 0]]
          d = SUT.determinantV a
      it "the determinant of A is 0" $ do
        d `shouldBe` 0
      it "and A is NOT invertible" $ do
        a `shouldSatisfy` (not . SUT.invertibleV)
    ...
    describe "Calculating the inverse of a matrix" $ do
      let a = makeVMatrix [[- 5, 2, 6, - 8], [1, - 5, 1, 8],
                           [7, 7, - 6, - 7], [1, - 3, 7, 4]]
          ba = makeVMatrix [[0.21805, 0.45113, 0.24060, - 0.04511],
                            [- 0.80827, - 1.45677, - 0.44361, 0.52068],
                            [- 0.07895, - 0.22368, - 0.05263, 0.19737],
                            [- 0.52256, - 0.81391, - 0.30075, 0.30639]]
          bi = SUT.inverseV a
          d  = SUT.determinantV a
          c  = SUT.cofactorV a (RowIndex 2) (ColumnIndex 3)
          c2 = SUT.cofactorV a (RowIndex 3) (ColumnIndex 2)
      it "the determinant of A is 532" $ do
        d `shouldBe` 532
      it "cofactor of A at [2,3] is -160" $ do
        c `shouldBe` - 160
      it "B [3,2] is -160/532" $ do
        SUT.getAtV bi (RowIndex 3) (ColumnIndex 2) `shouldBe` (-160)/532
      it "cofactor of A at [3,2] is 105" $ do
        c `shouldBe` - 160
      it "B [2,3] is 105/532" $ do
        SUT.getAtV bi (RowIndex 2) (ColumnIndex 3) `shouldBe` 105/532
      it "B is inverted" $ do
        bi `shouldBe` ba
    ...
    describe "Calculating the inverse of another matrix, A" $ do
      let a = makeVMatrix [[8, - 5, 9, 2], [7, 5, 6, 1],
                          [- 6, 0, 9, 6], [- 3, 0, - 9, - 4]]
          i = makeVMatrix [[-  0.15385, - 0.15385, - 0.28205, - 0.53846],
                          [- 0.07692,  0.12308,  0.02564,  0.03077],
                          [0.35897, 0.35897, 0.43590, 0.92308],
                          [- 0.69231, - 0.69231, - 0.76923, - 1.92308]]
      it "A is inverted" $ do
        SUT.inverseV a `shouldBe` i
    ...
    describe "Calculating the inverse of a third matrix, A" $ do
      let a = makeVMatrix [[9, 3, 0, 9], [- 5, - 2, - 6, - 3],
                          [- 4, 9, 6, 4], [- 7, 6, 6, 2]]
          i = makeVMatrix [[- 0.04074, - 0.07778, 0.14444, - 0.22222],
                          [- 0.07778, 0.03333, 0.36667, - 0.33333],
                          [- 0.02901, - 0.14630, - 0.10926, 0.12963],
                          [0.17778, 0.06667, - 0.26667, 0.33333]]
      it "A is inverted" $ do
    ...
    describe "Multiplying a product by its inverse" $ do
      let a = makeVMatrix [[3, - 9, 7, 3], [3, - 8, 2, - 9],
                            [- 4, 4, 4, 1], [- 6, 5, - 1, 1]]
          b = makeVMatrix [[8, 2, 2, 2], [3, - 1, 7, 0],
                            [7, 0, 5, 4], [6, - 2, 0, 5]]
          c = SUT.mulV a b
      it "will give us the same matrix we started with" $ do
        a `shouldBe` SUT.mulV c (SUT.inverseV b)
