...
mod test {
    ...
    fn a_triangle_has_a_bounding_box() {
        ...
        let s = Shape::new_triangle(p1, p2, p3);

        let _s_id = world.scene.insert_object(s, None, None);
        world.build();

        assert_eq!(
            bounds(&world.get_object(0).kind),
            BoundingBox::new(Point::new(-3.0, -1.0, -4.0), Point::new(6.0, 7.0, 2.0))
        )
    }
    ...
    fn finding_the_normal_on_a_triangle() {
        ...
        let n1 = s.normal_at(&Point::new(0.0, 0.5, 0.0));
        let n2 = s.normal_at(&Point::new(-0.5, 0.75, 0.0));
        let n3 = s.normal_at(&Point::new(0.5, 0.25, 0.0));
